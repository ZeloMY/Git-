...
namespace _6适配器模式
{
    ...
    /// 有点亡羊补牢的意思~~
    /// 应用场景：一般用在后期的系统维护阶段，接口改变的情况下使用！
    /// 但是当我们设计一个系统，调用第三方组件时，第三方的组件往往和我们需要的接口是不一致的，此时就可以用适配器模式
    /// </summary>
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //实例二
            Player zhang = new AssimilationChinesePlayer("张幕阳");
            zhang.Attratk();
            zhang.Defend();
            Console.ReadKey();
        }
    ...
    class AssimilationChinesePlayer : Player
    {
        ...
        public AssimilationChinesePlayer(string name)
            : base(name)
        {
            this.Name = name;
